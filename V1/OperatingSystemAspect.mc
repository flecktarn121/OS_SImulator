...
int OperatingSystem_LongTermScheduler() {
 ...
 for (i=0; programList[i]!=((void *)0) && i<20; i++) {
  if((PID=OperatingSystem_CreateProcess(i)) == -3){
   ...
   PROGRAMS_DATA program = *programList[i];
   ComputerSystem_DebugMessage(104, 'e', program.executableName, "---invalid priority or size---");
...
void OperatingSystem_HandleSystemCall() {
 ...
 switch (systemCallID) {
  ...
  case SYSCALL_YIELD:
   ...
   if((pid = OperatingSystem_ExtractFromReadyToRun()) != -1){
