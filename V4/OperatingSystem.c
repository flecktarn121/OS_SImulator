...
int OperatingSystem_LongTermScheduler() {
	...
	while(OperatingSystem_IsThereANewProgram() == 1) {
		...
		if((PID=OperatingSystem_CreateProcess(i)) == NOFREEENTRY){
			...
			PROGRAMS_DATA program = *programList[i];
			OperatingSystem_ShowTime(ERROR);
			ComputerSystem_DebugMessage(144, ERROR, program.executableName);
		}else{
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	// Obtain enough memory space
 	if((partition=OperatingSystem_ObtainMainMemory(processSize, PID)) == TOOBIGPROCESS){
	...
	partitionsTable[partition].occupied = true;
	partitionsTable[partition].PID = PID;
	OperatingSystem_ShowTime(SYSPROC);
	ComputerSystem_DebugMessage(143, SYSMEM, partition, loadingPhysicalAddress, partitionsTable[partition].size, PID, programList[processTable[PID].programListIndex]->executableName);
...
int OperatingSystem_ObtainMainMemory(int processSize, int PID) {
	if (processSize>MAINMEMORYSECTIONSIZE)
		return TOOBIGPROCESS;
	int i;
	int selectedAddress = MEMORYFULL;
	int freeSpace = INT_MAX; //vriable used to store the space left in the chosen block
	...
	if(selectedAddress != MEMORYFULL){
		OperatingSystem_ShowTime(SYSPROC);
		ComputerSystem_DebugMessage(142,SYSMEM,PID,programList[processTable[PID].programListIndex]->executableName, partitionsTable[selectedAddress].size);
	}
...
void OperatingSystem_HandleException() {
	...
	ComputerSystem_DebugMessage(140,INTERRUPT,executingProcessID, programList[executingProcessID]->executableName, exceptionName);
	//ComputerSystem
...
char* OperatingSystem_GetExceptionName(int exceptionType){
	return exceptionNames[exceptionType];
}
...
void OperatingSystem_ReleaseMainMemory(int PID){
	...
	for(i = 0; i < PARTITIONTABLEMAXSIZE; i++){
		if(partitionsTable[i].PID == PID && partitionsTable[i].occupied){			
			partitionsTable[i].occupied = false;
			...
			OperatingSystem_ShowTime(SYSPROC);
			ComputerSystem_DebugMessage(145,SYSMEM,i,address, size, PID,name);			
...
void OperatingSystem_HandleSystemCall() {
	...
	switch (systemCallID) {
		...
		default:
			OperatingSystem_ShowTime(SYSPROC);
			ComputerSystem_DebugMessage(141,INTERRUPT,executingProcessID, programList[executingProcessID]->executableName, systemCallID);
			OperatingSystem_TerminateProcess();
	}
}
