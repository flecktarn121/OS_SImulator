...
int OperatingSystem_LongTermScheduler() {
 ...
 while(OperatingSystem_IsThereANewProgram() == 1) {
  ...
  if((PID=OperatingSystem_CreateProcess(i)) == -3){
   ...
   PROGRAMS_DATA program = *programList[i];
   OperatingSystem_ShowTime('e');
   ComputerSystem_DebugMessage(144, 'e', program.executableName);
  }else{
...
_Bool OperatingSystem_CheckPriority(int currentPID, int candidatePID){
 ...
 }else if((processTable[currentPID].queueID == 0) && (processTable[candidatePID].queueID == 1)){
  return 0;
 }else if((processTable[currentPID].queueID == 1) && (processTable[candidatePID].queueID == 0)){
  return 1;
 }
 return 0;
}
